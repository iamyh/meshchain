...
#确保PATH里面已经追加gradle，譬如PATH=$JAVA_HOME/bin:$GRADLE_HOME/bin:$PATH
gradle build
cd dist
...
步骤二：生成每个ip的tar.gz压缩包，需要在对应的机器ip上面部署，部署的目录可以任意。通过scp或者其他方式到对应机器任一目录，执行：
...
步骤三：proxyConfig.tar.gz中的文件需要拷贝到[relay中继](https://github.com/FISCO-BCOS/meshchain.git) conf/下面的对应文件，执行
...
# 拷贝步骤二生成的proxyConfig.tar.gz到当前目录
tar -zxvf proxyConfig.tar.gz && cp proxyConfig/* conf/ && rm proxyConfig -rf
